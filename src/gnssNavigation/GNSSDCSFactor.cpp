...
namespace gtsam {
//***************************************************************************
void GNSSDCSFactor::print(const std::string& s,
...
Vector GNSSDCSFactor::whitenedError(const gtsam::Values& x,
        ...
        const int iter_count = iter_count_.load(std::memory_order_relaxed);

        // check range residaul
        if (std::pow(res(0),2) < k_(0)  || iter_count < 2)
        {
                v_range = model_(0,0);
        }
        else
        ...
        if (std::pow(res(1),2) < k_(1) || iter_count < 2)
        {
                v_phase = model_(1,1);
        }
        else
