...
bool nonBiasStates::equals(const nonBiasStates &q, double tol) const {
        return (std::fabs(x() - q.x()) < tol && std::fabs(y() - q.y()) < tol &&
                std::fabs(z() - q.z())<tol && std::fabs(cb() - q.cb())< tol &&
                std::fabs(tz() - q.tz())<tol);
}

void nonBiasStates::print(const std::string& s) const {
...
/* ************************************************************************* */
std::ostream &operator<<(std::ostream &os, const nonBiasStates& p) {
...
double norm5(const nonBiasStates &p, OptionalJacobian<1, 5> H) {
        double r = std::sqrt(p.x() * p.x() + p.y() * p.y() + p.z() * p.z()) + p.cb() + p.tz();
        if (H) {
                if (std::fabs(r) > 1e-10)
